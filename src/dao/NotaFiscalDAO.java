...
public class NotaFiscalDAO {
        ...
        try (PreparedStatement pst = conex.con.prepareStatement(query)) {
            ...
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    ...
                    nota.setNota_situacao(rs.getString("nota_situacao"));
                    return nota;
                } else {
    ...
    public void adicionarNotaFiscal(NotaFiscal notaFiscal) {
        ...
        try (PreparedStatement preparedStatement = conex.con.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                for (Item item : itens) {
                    if (notaFiscal.getNota_operacao().equals("SAIDA")) {
                        item.setQtd_mov(-item.getQtd_mov());
                    }
                    itemService.salvarItem(item, idGerado); // Usar o ID gerado ao salvar os itens

                    ProdutoDao produtoDao = new ProdutoDao();
                    produtoDao.atualizarSaldoProduto(item.getId_prod_ent());
                }
    ...
    public boolean expedirNotaFiscal(NotaFiscal notaFiscal) {
        conex.conexao();
        try (PreparedStatement pst = conex.con.prepareStatement(
                "UPDATE public.nota\n"
                + "SET   nota_nota=?, nota_data= ? , nota_hora= ? , nota_observacao= ? , nota_registro= ? , \n"
                + "nota_situacao= ? , nota_chave= ? ,     nota_usu= ? , \n"
                + "motorista= ? , placa= ? , uf= ? , quantidade= ? , especie= ? , numeracao= ? , pesobruto= ? , pesoliquido= ? ,  datavariavel= ? \n"
                + "WHERE id_nota=? ;")) {

            pst.setString(1, notaFiscal.getNota_nota());
            pst.setString(2, notaFiscal.getNota_data());
            pst.setString(3, notaFiscal.getNota_hora());
            pst.setString(4, notaFiscal.getNota_observacao());
            pst.setString(5, notaFiscal.getNota_data());
            pst.setString(6, notaFiscal.getNota_situacao());
            pst.setString(7, notaFiscal.getNota_chave());
            pst.setString(8, notaFiscal.getNota_usuario());
            pst.setString(9, notaFiscal.getTransporteModel().getMotorista());
            pst.setString(10, notaFiscal.getTransporteModel().getPlaca());
            pst.setString(11, notaFiscal.getTransporteModel().getUf());
            pst.setString(12, notaFiscal.getTransporteModel().getQuantidade());
            pst.setString(13, notaFiscal.getTransporteModel().getEspecie());
